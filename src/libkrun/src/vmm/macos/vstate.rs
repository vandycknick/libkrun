...
use arch::guest_memory::{RamPermissions, RamRegion, ReclaimError, ReclaimLedger};
...
impl Vm {
    ...
    pub fn memory_init(
        ...
        let initialization = self
            .hvf_vm
            .begin_memory_initialization(reclaim_ledger)
        ...
        for (host_addr, guest_start, len) in mappings {
            ...
            if let Err(error) = self.hvf_vm.map_memory(host_addr, guest_start, len) {
                if let Err(rollback_error) = self.rollback_memory_mappings(&mapped) {
                    initialization.fail_closed("failed to roll back HVF memory initialization");
...
pub(crate) fn build_reclaim_ledger(
    guest_mem: &GuestMemoryMmap,
    mem_info: &ArchMemoryInfo,
) -> Result<ReclaimLedger> {
    let granule = u64::try_from(mem_info.page_size)
        .map_err(|_| Error::ReclaimLayout(ReclaimError::MetadataLimit))?;
    ...
    let host_addr = guest_mem
        ...
        .map_err(Error::GuestMemoryMmap)?;
    let mut ledger = ReclaimLedger::new(granule).map_err(Error::ReclaimLayout)?;
...
mod tests {
    ...
    use crate::vmm::macos::vstate::{Vcpu, Vm, build_reclaim_ledger, memory_mappings};
    #[cfg(target_arch = "x86_64")]
    use crate::vmm::macos::vstate::{VcpuEvent, VcpuHandle, VcpuResponse};
    use arch::ArchMemoryInfo;
    use arch::aarch64::layout::DRAM_MEM_START_EFI;
    use arch::guest_memory::ReclaimError;
    use devices::legacy::VcpuList;
    ...
    fn test_reclaim_layout_excludes_shm_region() {
        ...
        let ledger = build_reclaim_ledger(&guest_mem, &mem_info).unwrap();
        assert_eq!(ledger.granule(), 0x4000);
        assert_eq!(ledger.generation_at(shm_start), Err(ReclaimError::NotRam));
    }
    ...
    fn test_file_backed_ram_is_mapped_but_not_reclaimable() {
        ...
        let ledger = build_reclaim_ledger(&guest_mem, &mem_info).unwrap();
        assert_eq!(ledger.generation_at(ram_start), Err(ReclaimError::NotRam));
    ...
    fn test_vm_memory_init_rejects_duplicate_before_mapping() {
        ...
        let reclaim_ledger = build_reclaim_ledger(&gm, &mem_info).expect("invalid RAM layout");
        vm.memory_init(&gm, reclaim_ledger)
        ...
        let second_ledger =
            build_reclaim_ledger(&second_gm, &second_mem_info).expect("invalid second RAM layout");
        assert!(matches!(
            vm.memory_init(&second_gm, second_ledger),
