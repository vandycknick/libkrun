...
use arch::guest_memory::ReclaimLedger;
use log::debug;
use reclaim::{
    FaultResolution, InitializationReservation, ReclaimQualification, ReclaimState,
    ReclaimStateError,
};
...
impl HvfVm {
    ...
    pub fn begin_memory_initialization(
        &self,
        ledger: ReclaimLedger,
    ) -> Result<InitializationReservation<'_>, Error> {
        self.reclaim_state
            .begin_initialization(ledger)
...
pub struct HvfVcpu<'a> {
    ...
    #[cfg(test)]
    after_hvf_exit_barrier: Option<Arc<std::sync::Barrier>>,
}
...
impl HvfVcpu<'_> {
    pub fn new(mpidr: u64, nested_enabled: bool) -> Result<Self, Error> {
        ...
        Ok(Self {
            ...
            #[cfg(test)]
            after_hvf_exit_barrier: None,
        })
    }

    #[cfg(test)]
    fn set_after_hvf_exit_barrier(&mut self, barrier: Arc<std::sync::Barrier>) {
        self.after_hvf_exit_barrier = Some(barrier);
    }
    ...
    pub fn run(
        ...
        let entered_generation = reclaim_state
            .vcpu_run_generation()
            .map_err(Error::ReclaimState)?;
        ...
        #[cfg(test)]
        if let Some(barrier) = self.after_hvf_exit_barrier.take() {
            barrier.wait();
        }
            ...
            EC_INSTRUCTIONABORT | EC_INSTRUCTIONABORT_CURRENT => {
                    ...
                    && let Some(generation) = entered_generation
                {
                    match reclaim_state
                        ...
                        .map_err(Error::ReclaimState)?
                    {
                        FaultResolution::Restored | FaultResolution::Stale => {
