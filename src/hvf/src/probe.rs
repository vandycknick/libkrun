...
use crate::discard::{PageState, advise_free, native_page_size};
use crate::reclaim::ReclaimQualification;

pub(crate) const DATA_SIZE: usize = 2 * 1024 * 1024;
...
pub(crate) const PSTATE_EL1H_MASKED: u64 = 0x3c5;
...
pub(crate) const TOUCH_PROGRAM: [u32; 9] = [
    0xf900_0003,
    0xf940_0005,
    0x8b05_0084,
    0x9100_0463,
...
impl ScratchMappings {
    ...
    fn release_data(&mut self, page_size: usize, advise: bool) -> Result<(), String> {
        ...
        let advice = if advise {
            unsafe { advise_free(self.data.address, DATA_SIZE, page_size) }
                .map_err(|error| format!("scratch page-wise MADV_FREE: {error}"))
        } else {
            Ok(())
        };
        if unsafe {
        ...
        {
            return Err("HVF rejected scratch data remap".to_string());
        }
...
fn run_owned_reclaim_probe(
    ...
    let page_size = match native_page_size() {
        Ok(size) if DATA_SIZE.is_multiple_of(size) => size,
        _ => {
...
fn run_probe_steps(
    ...
    // Reading the payload here would populate host PTEs and mask the bulk
    // advice failure on guest-only writes. Inspect metadata until after advice.
    let touched = match memory_sample() {
        Ok(sample) => sample,
        Err(error) => return inconclusive_report_with_samples(started, baseline, error),
    };
    let pages_before = match PageState::sample(mappings.data.address, DATA_SIZE, page_size) {
        Ok(pages) => pages,
        Err(error) => {
            return inconclusive_report_with_touched(started, baseline, touched, error.to_string());
        }
    };
    if pages_before.modified != DATA_SIZE / page_size
        || pages_before.resident != DATA_SIZE / page_size
    {
        return report_with_samples(
            ...
            None,
            format!(
                "scratch guest did not leave every page resident and modified: {pages_before:?}"
            ),
        );
    }

    if let Err(error) = mappings.release_data(page_size, mode == ProbeMode::Release) {
    ...
    let pages_after = match PageState::sample(mappings.data.address, DATA_SIZE, page_size) {
        Ok(pages) => pages,
        Err(error) => {
            return inconclusive_report_with_release(
                started,
                baseline,
                touched,
                released,
    ...
    if mode == ProbeMode::NegativeControl {
        let absent = pages_after.modified == pages_before.pages
            && host_pages_match(&mappings.data, page_size, first_seed);
        return report_with_samples(
            if absent {
            ...
            released,
            None,
            format!("unmap/remap-only control: before={pages_before:?} after={pages_after:?}"),
        );
    }
    if !pages_after.is_discardable() {
        return report_with_samples(
            ReclaimQualification::Failed,
            started,
            baseline,
            ...
            None,
            format!(
                "page-wise advice left referenced, modified or compressed backing: {pages_after:?}"
            ),
        );
    }
    // The range is already remapped; the second pass proves the guest can reuse
    // it and that its writes reach the host through the restored mapping.
    ...
    report_with_samples(
        ...
        Some(retouched),
        format!(
            "page-wise discard state verified: before={pages_before:?} after={pages_after:?}; guest reuse verified; pressure discard not tested at startup"
        ),
    )
}
...
pub(crate) fn expected_checksum(seed: u64, pages: usize) -> u64 {
...
mod tests {
    ...
    use crate::probe::{
        DATA_SIZE, choose_scratch_gpas, expected_checksum, requires_emergency_destroy,
    };
    ...
    fn task_vm_info_layout_matches_public_sdk_headers() {
        ...
        assert_eq!(
            size_of::<crate::probe::TaskVmInfoRev1>() / size_of::<i32>(),
            values["rev1_count"]
        );
        assert!(values["full_count"] * size_of::<i32>() <= values["full_size"]);
        assert_eq!(
            std::mem::offset_of!(crate::probe::TaskVmInfoRev1, resident_size),
            values["resident_offset"]
        );
        assert_eq!(
            std::mem::offset_of!(crate::probe::TaskVmInfoRev1, reusable),
            values["reusable_offset"]
        );
        assert_eq!(
            std::mem::offset_of!(crate::probe::TaskVmInfoRev1, phys_footprint),
    ...
    fn real_hvf_guest_touch_positive_and_negative_controls() {
        ...
        eprintln!(
            "ipa_bits={ipa_bits} host_page_size={}",
            crate::discard::native_page_size().unwrap()
        );
